...
| [operator-registry.md](operator-registry.md) | **设计稿**:万物皆注册算子——`search` 只是一条最佳实践,`find`/`sed`/`grep`/`sh` 平级注册;注册契约(`Operator` 子类:格式契约 `accepts`/`emits` / 参数签名 / caps / 降级实现)+ **权限范围**(Claude/Codex 式:能力 capability × 策略 policy,默认 `read-only`、`EXEC` 默认关、放行也在沙箱里),给 pipeline §9 的安全洞一道正式围栏 |
| [pipeline-runtime-optimize.md](pipeline-runtime-optimize.md) | **设计稿(后端)**:**管道不自己跑**——整条被降级到一个 **pipeline runtime**;runtime 是**开放集**(今 DuckDB `WITH` / bash pipeline 两个;`run` 是两者的兜底、不是第三个 runtime;可扩展,§10 给加新 runtime 的四点契约)。**关键分层(§1.1):runtime ⊥ 算子内部的引擎后端**——DuckDB/bash 是承载整条管道的 runtime,LanceDB/duck-vss 是 `search` 一个算子背后的后端,两层正交、别混。三种降级手段的代价阶梯(**同 runtime 原生翻译 0 成本 > 切段一次物化 > 内联桥每批 marshal**)、runtime 指派 = 一条最短路(对 runtime 数量无感)、连续同 runtime 段融合、`grep` 多写一版 `optimize_duck`(翻成 `WHERE`)如何把切换点消成 0;外加双 optimize_* 的语义等价风险(differential test 是准入条件)|
| [operator-plugin.md](operator-plugin.md) | **设计稿(作者视角)**:一个算子 = 一个 **`Operator` 子类**——声明放类属性、行为放方法覆写,层次只有一层(`Source` / `ExternalCommand` 只预设默认值)。**为什么是继承不是记录**:服务型要 `self` 存 handle、三条降级路要共享私有方法、一父两子复用(`Search` → `LanceSearch`/`VssSearch`)、`parse_args` 按参数变 caps。**三个正交轴,全从「覆写了什么」推导**:① 格式契约 `accepts`/`emits`(不用 `kind`)+ coercion;② 覆写 `start`/`stop` = 服务型;③ **`run` 必修基线**(任何 runtime 都能靠内联桥承载)+ **可选**的 `optimize_*`(覆写哪几个 = 免掉哪个 runtime 的桥、会不会成为**切换点**;都不覆写就靠 run 兜底——能跑最贵)。外加有界性塌缩成宿主属性(**`tail -f` 进 duck = 编译期报错**;watermark/checkpoint/keyed state 明确不借)、`ctx` capability 唯一入口、三个例子(`Grep`/`Search`/`Jq`)+ 测试 + 诚实代价 |
