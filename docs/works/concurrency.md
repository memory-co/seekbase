...
| **b 异步** | `t = write.enqueue(op); return t`(立刻回 `pending`,worker 事后改 `done`) | 写不阻塞、可 fire-and-forget | **重新引入最终一致窗口**(insert 返回后、worker 未处理前 `search()` 搜不到,需 `wait(ticket)`)——即把 **outbox** 请回来 |
