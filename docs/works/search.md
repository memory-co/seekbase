...
| 删除 | 软删只 `UPDATE deleted_ds`(非索引列),行/向量**留在索引里** | 同上;删的行由**查询侧的 as-of 谓词**裁掉(现在 `deleted_ds IS NULL`,回溯则按 ds 存活)——行还在索引,所以时光机回得去 |
...
- **`<可见性谓词>` = 结构化侧同一个 `_asof_conds`**:两支候选和外层可见性视图**共用**同一 as-of 谓词(现在 `deleted_ds IS NULL`;as-of D 则 `ds<=D AND (deleted_ds IS NULL OR deleted_ds>D)`)。所以 **search 尊重 `ds` 窗口**:软删的行仍在索引里(软删不重建索引),回溯到它还活着的那天照样能被搜到,as-of now 才被过滤掉——查和搜对齐,不再有"搜不到历史"的不对称。
- **`cand`(over-fetch):** HNSW/BM25 是"先取 top-k、再按谓词过滤",过滤发生在**取 top-k 之后**,历史查询里被排除的"当时还没建/已删"行会挤占名额、导致少返回。所以历史路径(`ds_end` 非空)把候选池放大 `_OVERFETCH`×(现在路径不变,`cand=k`),实测 ×2 即可补回召回(store_service.py `_OVERFETCH`)。
- 融合出的 `(pk, score)` 如何缝回外层 SQL、暴露成 `_score_<列>`,见 §3.3。
...
- [time_machine.md](time_machine.md):search 候选与外层视图共用同一 as-of 谓词,回溯到历史存活状态。
