...
> **状态:核心完整(管道 M1 + 单表同步写)。** 管道 `query`(SQL 缺省 + `search`/`scan`/`grep` 算子段,整条编译成一条 `WITH` SQL;duck runtime)、`ds` 时间窗、同步 ticket 写(`insert`/`delete`,**主键写一次、重复报错**)、文件镜像(每表 `<表>.jsonl` + `rebuild`)、检索后端(DuckDB `vss`+`fts` 就地长在业务表上;引擎按设计可插拔,LanceDB 后端未落)、两种使用形态,今天都能跑。**delete 只软删 `deleted_ds` 墓碑、历史永久保留,没有物理删/vacuum**。完整设计见 [DESIGN.md](DESIGN.md)。

> **🚧 后续方向(设计已定,未落):** bash runtime(`sh`/`jq` 等 EXEC 算子 + 能力×策略沙箱限权,[operator-registry.md](docs/works/operator-registry.md))、runtime 指派/融合优化([pipeline-runtime-optimize.md](docs/works/pipeline-runtime-optimize.md))、流式摄取([pipeline-streaming.md](docs/works/pipeline-streaming.md))、LanceDB 检索后端。旧的 `search()` UDF 语法已退休。
