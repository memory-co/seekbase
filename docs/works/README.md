...
| [pipeline-runtime-optimize.md](pipeline-runtime-optimize.md) | **设计稿(后端)**:**管道不自己跑**——整条被降级成 DuckDB 的 `WITH` 链或 bash 的 pipeline,于是跨段优化 / 背压都是白送的。三种降级手段的代价阶梯(**同宿主原生翻译 0 成本 > 切段一次物化 > vtab 桥每批 marshal**)、宿主指派 = 一条最短路、连续同宿主段融合、`grep` 多写一版 `compile_duck`(翻成 `WHERE`)如何把切换点消成 0、`search` 的两个 compile_*(bash 调 LanceDB SDK / duck 走官方集成);外加双 compile_* 的语义等价风险(differential test 是准入条件)|
