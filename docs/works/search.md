...
### 3.3 算候选 + 缝回(临时表 + LEFT JOIN 重放视图)

executor 对每个抽出的 search:embed 文本 + jieba 分词 → 在该列派生表上跑 §4 的 **RRF**,得到 `[(pk, 融合分)]`。duck 把它灌进一张临时表 `_sb_s_<i>(pk, score)`,再在目标表的**重放视图**上 `LEFT JOIN` 它,把 `score` 暴露成 `_score_<列>`(单 search 另附 `_score` 别名):

```sql
CREATE OR REPLACE TEMP VIEW cards AS
SELECT base.*, _s0.score AS "_score_issue", _s0.score AS "_score"
FROM ( …重放视图:as-of 的最新存活行(见 time_machine.md)… ) base
...
> **一句话**:`search()` 被拆成「一个布尔占位符(进 `WHERE`)+ 一张分数临时表(`LEFT JOIN` 进重放视图)」;真正的向量/全文检索(§4 RRF)在旁路完成,以 `(pk, score)` 缝回,外层永远是原生 DuckDB SQL。
