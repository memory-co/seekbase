...
> 状态:**设计**(未落地)。当前实现是 `TicketService` 里的一个内存 dict;本文是把它换成一个**独立、落盘、状态-only 的回执日志**的设计。对外用法见 [api/insert.md](../api/insert.md) 的 `ticket` 字段。
...
- **`TicketService`**(`service/ticket_service.py`):`issue(op, matched?, stats?)` → append 一行、返回 `Ticket`;`status(id)` → 定位分区扫描。持有 tickets 目录(+ bridge,append 走单写者串行,和文件镜像一致)。
- **`Ticket`**(`struct/ticket.py`):数据对象不变(`id/op/state/matched/stats` + `to_wire`/`from_wire`);JSONL 行就是 `to_wire()`。
- **谁调**:`write_service` / `admin_service` 写完调 `tickets.issue`;`status` 端点(`api/writes.py`)/ `client.write_status` 调 `tickets.status`。

## 8. 与其他文档

- [store.md](store.md):写入流水(files-first → duck.db),ticket 是其最后一步。
- [architecture.md](architecture.md):service 分层里 TicketService 的位置。
