...
> 状态:**Bridge 已落(M1);a 模式写 worker + 批处理已落(§4/§5/§6,`service/write_service.py`);读写分离为设计(未落,§3)**。本文讲 seekbase 怎么在 async 世界里安全跑同步的 DuckDB:为什么有 Bridge、为什么读会排在写后面、怎么把读拆出去、以及把写收敛成一条**看得见生命周期**的 worker(队列 + ticket)。
