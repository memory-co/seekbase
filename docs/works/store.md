...
- **LanceDB = 向量派生**:每个 `searchable` 列各自一个 `(pk, vector)` 索引,`search(列, …)` 只搜那一列。**派生索引,可重建**。
...
<data_dir>/
  ...
  lance/                                     # LanceDB:每个 (表, searchable 列) 一个向量表 vec_<表>__<列>
...
- **派生 DuckDB**:**INSERT 一条 del 事件**(pk + `deleted_ds`/`deleted_at` + `_seq`,business 列 NULL)——纯 append,不改任何已写的行。
