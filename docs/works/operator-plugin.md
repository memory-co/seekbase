...
>
> **但 seekbase 不自己跑管道**:整条管道被**降级**成 DuckDB 的 `WITH` 链或 bash 的 pipeline([pipeline-runtime-optimize.md](pipeline-runtime-optimize.md))。所以算子的**首要**作业不是「怎么算」,而是**「我在某个宿主里长什么样」**——覆写 `native_duckdb` / `native_bash`(§3.2)。两个都不覆写,才退回 Python 的 `run`,由框架包成 vtab(能跑,最贵)。
>
...
class Grep(Operator):
    ...
    def native_duckdb(self, prev, args):      # 我在 duck 宿主里长这样(§3.2)
        return f"SELECT * FROM {prev} WHERE {self._sql_pred(args)}"

    def native_bash(self, args):              # 我在 bash 宿主里长这样
...
class Operator(ABC):
    ...
    def native_duckdb(self, prev: str, args) -> str:  raise NotSupported
    def native_bash(self, args) -> list[str]:         raise NotSupported

    # ── 保底:两个 native 都没有时走这条(被包成 vtab)────────
...
| `ExternalCommand(Operator)` | `caps = {Cap.EXEC}`,并**替你实现 `native_bash`**(按 `argv` 模板渲染) | 只填 `argv` + 格式 |
...
- **覆写点之间无法共享逻辑**。`native_duckdb`、`native_bash`、`run` 三条路要产生**同一个语义**(§3.2),它们必然共用参数校验、谓词构造、列名推导。三个独立函数只能靠模块级 helper 或复制粘贴;有 `self` 就是一个私有方法。
...
- **服务型算子(如 `search`)**:背后是一个**常驻服务**——向量引擎连接、**载进 RAM 的 HNSW 索引**、embedder 连接池。这些**开一次、复用多次**,**绝不能每次调用都重开**:
  ...
  | `run` / `native_*` | 每次调用 | **复用 `self` 上的 handle**,绝不重开 |
...
```python
class Grep(Operator):
    def _regex(self, args):                                  # ★ 三条路共用一份参数逻辑
        return args.pattern                                  #   (记录式做不到这件事)
    ...
    def run(self, in_table, args, ctx):                      # 保底:被包成 vtab(能跑,最贵)
...
| 一个 `native_*` | 在那个宿主里**免费**;在另一个宿主里是**切换点**(整条管道被劈开) |
| 两个 `native_*` | **永不成为切换点**——跟着上下文走,零成本 |

> **写第二个 native 的理由不是「能在两边跑」,是「不成为切换点」。** 一堆 duck 段中间夹一个只会 bash 的 `grep`,代价是整条管道劈成三截 + 一座 vtab 桥;覆写一个 `native_duckdb`(把 grep 能力整个翻译成 `WHERE`),代价直接归零。收益是复利的,见 [pipeline-runtime-optimize.md §5](pipeline-runtime-optimize.md)。
...
- **`ctx` 只在 `start` 和 `run` 里出现**。`native_duckdb`/`native_bash` **拿不到 `ctx`**——它们只生成代码,不碰外界;真正的资源访问发生在宿主里,由沙箱和策略层管(operator-registry §6)。
...
- **同理,「覆写了什么」也是推导的,不是声明的**:有没有常驻状态看有没有覆写 `start`;能进哪个宿主看覆写了哪个 `native_*`。**一律不设声明字段**——声明和实现能打架的地方,就是 bug 的产地。
...
> | **宿主降级** | 覆写了哪几个 `native_*` | 我能编译进哪个宿主? | 宿主指派 + 会不会成为切换点(§3.2) |
>
> ```
> Grep         = TABLE→TABLE   + 无状态      + native: duck & bash    永不成为切换点
> LanceSearch  = NONE →TABLE   + 服务型      + native: duck & bash    duck 走官方集成,bash 走 SDK
> Jq           = JSONL→JSONL   + 无状态      + native: bash           在 duck 宿主里是切换点(架 vtab)
> 你的第一个算子 = TABLE→TABLE  + 无状态      + native: 无             只覆写 run:能跑,最贵
> SQL 段        = TABLE→TABLE   + (非 plugin) + 天生 duck              整条链的锚点
> ```
>
> **最低门槛**:继承 `Operator`,实现 `out_schema` + `run`,收工。`start`/`stop` 和 `native_*` 都是**为了跑得便宜才覆写的第二档**。
...
class LanceSearch(Search):                                # 后端 A:LanceDB
    ...
    def native_bash(self, args):                          # bash 宿主:小命令直调 LanceDB SDK
        return ["seekbase-search", args.table, args.text, "--k", str(args.k)]

    def native_duckdb(self, prev, args):                  # duck 宿主:DuckDB×LanceDB 官方集成
...
class VssSearch(Search):                                  # 后端 B:DuckDB-vss + fts,同一父类
    ...
    def native_duckdb(self, prev, args):                  # 本来就在 duck 里,最自然
...
class Jq(ExternalCommand):
    name    = "jq"
    argv    = ["jq", "-c", "{script}"]                   # 基类据此替你实现 native_bash
...
用:`search cards '事故' | jq 'select(.severity>=3)' | SELECT kind, count(*) FROM _in GROUP BY kind`。默认 `read-only` 策略下 **`jq` 因 `EXEC` 被拒**(`ExternalCommand` 预设了 `caps={Cap.EXEC}`),要显式升级到 `sandboxed`/`trusted` 才能跑(operator-registry §6)。**只有 `native_bash`** ⇒ 它在 duck 宿主里是个切换点,编译器会为它架一座 vtab 桥。
...
authorize                算子 caps vs 策略 → allow / ask / deny(deny→编译期拒,管道不启动)
   │
compile                  宿主指派 → 调 native_duckdb / native_bash 生成代码;都没有就包 vtab 调 run
   │
...
- **多路 differential test(准入条件,非可选)**:同一份输入,分别**强制**走 `native_duckdb` / `native_bash` / `run`,断言逐行一致。三条路结果不同 = 同一条 query 换宿主换答案,这是本设计最危险的 bug(§3.2)。**父类可以直接提供这个测试基类**,子类只填输入——这是继承的又一处兑现。
...
- **降级测 `EXPLAIN`**:断言宿主指派符合预期(比如「加了 `native_duckdb` 之后切换点从 2 变 0」),否则性能回退无人察觉。
...
- **多路实现 = 多份维护 + 等价性风险**(§3.2)。只覆写一个 native 是完全正当的选择,它只意味着这个算子**是个切换点**——请在文档里写明,别让用户猜为什么某条管道突然变慢。
- **只覆写 `run` 不是错,是贵**:保底 vtab 一定跑得通,但它是**优化屏障**(DuckDB 看不穿桥、谓词推不进去、基数估计失真)+ 每批 marshal。热路径上的算子值得补一个 native。
...
- [pipeline-runtime-optimize.md](pipeline-runtime-optimize.md):**本文 `native_*` 的去处**——宿主指派、融合切段、vtab 桥、代价阶梯。作者视角看「覆写几个 native」,系统视角看「编译器怎么用它们省钱」。
