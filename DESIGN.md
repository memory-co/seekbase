...
### 6.4 async↔sync 桥与并发(`runtime/bridge.py`)
...
- **service 是唯一的用例入口,两个调用方都直连它**:HTTP 形态 `api/*.py` 直接调 `db.services.{read,write,admin,tickets}`(service 自己拼完整响应 + ticket);嵌入/远程形态走 `client → executor`,`LocalExecutor`(`client.py` 的 LocalExecutor)是**薄转发**(`op→service`,只为让 client 传输无关),`HttpExecutor`(`api/remote.py`)把 `Request` 序列化成 HTTP 打到那些 api。两条路复用同一批 service 方法,零重复。分层:`{api 直连 | client→executor} → service(领域服务 Store/Search/File + 用例服务 Write/Read/Admin)`,`runtime/`(桥/时钟)在旁。**一切皆 service**:领域服务各自拥有一个子域端到端,用例服务只管跨子域的顺序+原子(files 先于 db)。
