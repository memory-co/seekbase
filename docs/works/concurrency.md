...
> 状态:**Bridge 已落(M1);读写分离 + 显式写管道为设计(未落)**。本文讲 seekbase 怎么在 async 世界里安全跑同步的 DuckDB:为什么有 Bridge、为什么读会排在写后面、怎么把读拆出去、以及把写收敛成一条**看得见生命周期**的管道(worker 循环 + ticket)。
...
今天 Bridge 串行化**一切** DuckDB 访问——`query` 也走 `bridge.run`。于是**读排在写后面**:一次 `search()` 会卡在前面的写、尤其是 **FTS 重建**(`create_fts_index` 是 O(表大小),大表要几百毫秒~秒级)之后。对读多写少的 memory 场景,这是主要的并发痛点。
...
## 4. 写管道:一条**看得见生命周期**的 worker

写侧收敛成一个显式的 `WritePipeline`——worker 是一段能读的循环,一次写的一生(入队 → 取出 → 执行 → 发 ticket → 唤醒)在源码里摊开,不埋在 executor 黑盒里:

```python
class WritePipeline:                       # 拥有:写连接 + worker + ticket 日志
    async def start(self):
        self._worker = asyncio.create_task(self._run())     # ★ worker 起来,看得见
    ...
    async def _run(self):                  # ★★ 写 worker 的一生
...
- **Bridge 不消失,沉到底层**:`_execute` 里真正阻塞的 DuckDB / 文件调用仍走 `bridge.run(...)`(不卡循环)。**Bridge = 「把阻塞活甩到线程」的底层原语;WritePipeline 的 worker = 你能看到的写生命周期**——一下一上,各司其职。
- **串行 + ticket 在一处合体**:队列的「多→一」是串行/单写者,循环末尾发 ticket 是完成标记——正是 Bridge 的写角色 + `TicketService` 该在一起的地方(见 [ticket.md](ticket.md))。写侧不再散在 `runtime/` 的 Bridge 和 `service/` 的 ticket 两头。
- **asyncio 原语(`Queue`/`Future`)只在事件循环线程内碰才安全**。本设计的 worker 是**协程**(`create_task`,**不是线程**),所以 `submit`(生产)和 `_run`(消费)都在循环线程上——**队列 / Future 从不跨线程**。唯一过线程边界的,是 `_execute` 里那句 `bridge.run(...)`(= `run_in_executor`,被祝福的过线程桥,内部 `wrap_future` 做线程安全完成通知);那条 DuckDB 线程**永远碰不到队列 / Future**。
...
| | 调用方 | 语义 | 代价 |
|---|---|---|---|
| **a 同步** | `return await pipeline.submit(op)` | 拿到即 `done`,**read-your-write 不变**(写完即可搜) | 调用方阻塞在 embed 网络 + 写 |
| **b 异步** | `t = pipeline.enqueue(op); return t`(立刻回 `pending`,worker 事后改 `done`) | 写不阻塞、可 fire-and-forget | **重新引入最终一致窗口**(insert 返回后、worker 未处理前 `search()` 搜不到,需 `wait(ticket)`)——即把 **outbox** 请回来 |

- **b 不再踩 HNSW 段错误**(worker 做的是完整 inline-embed 写,不是「先写 NULL 再 UPDATE」那个崩法),技术可行;但它是路线之争(同步简单 vs 异步吞吐)。
- **所以不用现在拍板**:先把显式 worker 建出来,a/b 是边缘策略,甚至两个都给。
...
```
读路径  query
  → 读 executor(小线程池)+ conn.cursor()      并发读,MVCC,不排在写后
写路径  insert / delete / rebuild
  → WritePipeline(写连接 + 单 worker 队列 + ticket 日志)
       worker: 队列多→一 → _execute(经 Bridge) → 发 ticket → 唤醒
Bridge  runtime/:把阻塞调用甩到线程的底层原语(读、写两侧都用)
```

- `WritePipeline` 放 `service/`(它是写的用例执行 + 生命周期);`Bridge` 留 `runtime/`(通用原语)。
- 写侧的 `WriteService`(校验/embed/顺序编排)与 `WritePipeline`(串行执行 + ticket)如何分工,落地时再定:要么 WriteService 把「一次写」打包成 `op` 交给 pipeline,要么两者合并成一条写管道(呼应「Bridge+Ticket 该在一起」)。
...
- [ticket.md](ticket.md):ticket 是写的完成标记,由 WritePipeline 的 worker 末尾发出。
