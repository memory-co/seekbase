...
```
                         SCHEMA(唯一声明源)
        ┌───────────────┬───────────────┬───────────────┐
   DuckDB 物理表     业务表上的        files 镜像      元数据列
   (结构化 DDL)      vss+fts 列/索引    (canonical)     ds/created_at/
...
- 列出可被 `search()` 检索的列;**必须是 `str` 列**(向量只 embed 文本、BM25 只索引文本,`json` / 数值 / 时间列不可搜)。声明了 → 该表多出 `_vec_<列>`/`_tok_<列>` 两列 + 索引;`insert` 时该列文本**自动 embed + jieba 分词**随行写入、`search()` **自动 hybrid 查**——写入方 / 查询方都不用碰检索栈(见 [search.md](search.md))。
- **有 `searchable` ⇒ 必须注入 embedder**(否则 `open` 时 `EmbedderInvalid`,早失败)。
- **没有 `searchable` 的表 = 纯结构化表**:零检索开销,连 `_vec_<列>`/`_tok_<列>` 列和 vss/fts 索引都不建。
- 这就是「`search()` 一等算子」的接线开关:声明与否决定一张表建不建 vss+fts 列。
...
| **vss + fts 索引** | 每个 `searchable` 列一个 HNSW 索引在 `_vec_<列>`(cosine);一个 BM25 索引盖住所有 `_tok_<列>`,`search(issue, …)` 用 `match_bm25(fields:=)` 限定该列、hybrid 融合(见 [search.md](search.md)) |
