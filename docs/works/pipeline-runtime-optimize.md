...
> 状态:**设计稿(pipeline 方向,未落)**。[pipeline-as-anything.md](pipeline-as-anything.md) 定的是**前端**(query = `stage | stage`);本文定**后端**:这根管道**不由 seekbase 自己执行**,而是被**降级(lowering)**到一个 **pipeline runtime(运行时/宿主)**。runtime 是一个**开放集**——今天有两个(**DuckDB 的 `WITH` 链**、**bash 的 pipeline**),后续可以再加(§10)。算子的 Python `run` **不是**第三个 runtime,而是**两个 runtime 都能承载的保底手段**(duck 里包成 vtab、bash 里读 stdin,§2)。我们不造管道运行时,我们造一个**面向多 runtime 的编译器**。
>
> 于是「优化」有了精确含义:**同一条 query 有多种降级方案(落到哪个 runtime、在哪切),成本差一个数量级,编译器要挑便宜的那个。** 本文就是这套挑法。
>
> 依赖:[pipeline-as-anything.md](pipeline-as-anything.md)(前端语法、`_in` ABI、§2.1「接缝才切」)、[operator-plugin.md](operator-plugin.md)(算子契约:`optimize_duck` / `optimize_bash` / 保底 `run`)。
...
| **③ 内联桥** | **永远可用**(算子总有 `run` 兜底) | 无(流式) | ❌ 是屏障 | **每批** | 中~高(随行数) |

- **③ 把「非当前 runtime 的一段」嵌进当前 runtime**,不切走。每个 runtime 有自己的内联形式:
  - **duck runtime 里 = 包成 vtab**(table function):DuckDB 把行喂进那段、收回结果。被嵌的可以是一条 bash 命令(`bash_vtab('jq …')`),也可以是算子的 Python `run`(回调)。
  - **bash runtime 里 = 包成读 stdin 的子进程**:那段从 **stdin** 收行、吐 stdout,当管道里一个普通命令。被嵌的可以是一段 duck SQL(`duckdb -c …`),也可以是 Python `run`。
  - **`run` 是这条的兜底**:一个算子哪怕没有当前 runtime 的 `optimize_*`,它总有 `run`,所以③**永远架得起来**——vtab(duck)或 stdin 子进程(bash)。这就是为什么③标「永远可用」。
...
- **只有 `run` 的段**(没覆写任何 `optimize_*`)不挑 runtime:落在哪个 runtime 就以 ③ 内联桥承载(§7)。
...
class Grep(Operator):
    ...
    def optimize_duck(self, prev, args):        # duck 宿主:整个翻译成 WHERE
        return f"SELECT * FROM {prev} WHERE regexp_matches({args.field}, {self._regex(args)!r})"

    def optimize_bash(self, args):              # bash 宿主:就是 grep 本身
        return ["grep", "-E", self._regex(args)]

    def run(self, in_data, args, ctx): ...     # 保底:两边都没有,框架包成 vtab(③)
```
...
| 只覆写 | 后果 |
|---|---|
| `run` | 落哪都走 ③ 内联桥(duck=vtab、bash=stdin);能跑,最贵 |
| 一个 `optimize_*` | 在那个宿主里免费,在另一个宿主里是**切换点** |
...
内联桥(§2 的③)有两种形式,取决于当前 runtime;被嵌的既可以是**另一 runtime 的原生代码**,也可以是算子的 **Python `run`**:

- **duck runtime 里 = vtab**:注册一个 **table in-out function**——吃上游关系当表参数,按批把行序列化进那段(Arrow IPC 优先,JSONL 退化),从它那儿解析回批。DuckDB 侧看到的是普通表函数,`FROM bash_vtab('jq …', TABLE s2)`。嵌 bash 命令时它 fork 子进程;嵌 `run` 时批回调就是 `run` 本身。
- **bash runtime 里 = 读 stdin 的子进程**:被嵌的那段从 **stdin** 收行、吐 stdout,当管道里一个普通命令。嵌 duck SQL 时是 `duckdb -c "COPY (SELECT … FROM read_json_auto('/dev/stdin')) TO '/dev/stdout' (FORMAT JSON)"`(DuckDB CLI 本就能读写 `/dev/stdin`/`/dev/stdout`);嵌 `run` 时是一个读 stdin 的小 Python 进程。
- **`run`-only 算子是退化情形**:它没有任何 `optimize_*`,所以落在哪个 runtime 就用那个 runtime 的内联形式承载(vtab 或 stdin 子进程)——这也是为什么③永远架得起来。
...
- [operator-plugin.md](operator-plugin.md):算子契约——`optimize_<runtime>`(当前 `optimize_duck` / `optimize_bash`)/ 保底 `run`;§3.3 的 `bounded` 传播按本文 §9 简化。
