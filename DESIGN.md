...
seekbase/                      # 仓库根
  ...
  seekbase/                    # 包(flat,不用 src/)
    ...
    _engine/
      ...
      # 时光机 = duck.py 可见性视图(ds/deleted_ds);ds_start/ds_end 是 query 参数  [M4 已落]
    embedders/
...
时光机用**离线大数据那套分区**实现,不靠谓词改写。**完整设计(创建/删除两对日期字段、可见性谓词、完备性证明)见 [works/time_machine.md](docs/works/time_machine.md)**;这里给要点。
