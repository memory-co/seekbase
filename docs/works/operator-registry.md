...
class Grep(Operator):
    ...
    def native_duckdb(self, prev, args): ...   # 我在 duck 宿主里长什么样
    def native_bash(self, args):         ...   # 我在 bash 宿主里长什么样
...
- **`run` 收 `ctx`**:上下文里带**已授的权限边界**(可读哪些路径、能否联网、沙箱句柄),算子只能在边界内动手——纵深防御,不只靠编译期检查。而 `native_*` **拿不到 `ctx`**:它们只生成代码,真正的资源访问发生在宿主里,由沙箱管(§6.3)。
