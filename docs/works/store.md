...
> 状态:**M2/M3/M5 已落**(files→行→检索:文件 append、DuckDB 事件行、outbox→`vss`+`fts` 异步兑现;`rebuild` replay)。**单引擎:结构化 + 向量 + 全文全在一个 `duck.db`,没有 LanceDB。delete 永久墓碑、无物理删 / vacuum**(文件真·纯 append,零例外)。本文定下两层存储的角色、files 目录结构(**按天分区、每表一个 append 日志**)、insert 的「文件最先」原子性顺序,以及用 files 校准派生层的机制。

## 1. 两层存储,一个端口

一次写入落到两层:canonical 文件 + 派生的 DuckDB(单文件,内含事件表 + 检索派生表)。
...
```
    ...
    (canonical)               ┌──────────────┴──────────────┐
   可 grep/diff             事件表                    检索派生表
                        过滤·聚合·join·重放      vss(向量)+ fts(全文)
          └────────── 派生自 files,可从 files 重建 ──────────┘
```

- **files = canonical(权威)**:数据的真相。纯文本 JSONL、可 `grep`/`cat`/`diff`/git,是可审计、可信任的底线。
- **DuckDB = 派生(单引擎)**,内含两块,都可从 files 重建:
  - **事件表** `_sb_<表>`:append-only put/del 事件 + `_seq`,行存取、过滤、聚合、join、`ds` 时间窗重放视图(结构化 + 时光机)。
  - **检索派生表** `_sb_search_<表>__<列>(pk, txt, tok, vec)`:每个 `searchable` 列一个,`vss`/HNSW 索引 `vec`(向量语义)+ `fts`/BM25 索引 `tok`(全文,中文经 jieba 分词)。`search(列, …)` 只搜那一列、hybrid 融合。**这就是原来 LanceDB 的角色**,现在搬进同一个 `duck.db`。
...
<data_dir>/
  files/
    ...
    ds=20260706/                             # 当天没写 rounds,就没 rounds.jsonl
      cards.jsonl
  duck.db                                    # DuckDB 单文件:事件表(ds/created_at/deleted_ds/deleted_at)
                                             #   + _sb_outbox + 检索派生表 _sb_search_<表>__<列>(vss+fts)
...
- **重复主键 = 再 append 一行**(一个新版本);**派生 DuckDB 也纯 append**(每个版本一条事件),查询视图按 `_seq` 现算最新版本——和「改 = 追加新行」的 insert-only 天然一致,文件与派生表都从不回改已写的行。
- **主键(`primary`)保留**,但**不进文件路径 / 不做文件名**:它只在派生层做行标识、多版本现算最新、删除定位。
...
检索派生表虽然和事件表同在一个 `duck.db`,但兑现要 embed(网络)+ FTS 重建,仍是异步。解法不是两阶段提交,而是**定死写入顺序 + 用 canonical 校准**。`insert(row)` 三步:
...
① files 最先 —— append 落盘
     以 O_APPEND 打开 ds=今天/<表>.jsonl → 写一行 `json.dumps(record)+"\n"` → fsync
        ...
        ▼
② 一个 DuckDB 事务(原子)
     写事件行 + 追加 _sb_outbox 一行(检索作业)   ← 队列就在 DuckDB 里,和事件行同事务
        │  commit
        ▼
③ 后台 consumer(commit 后异步)
     取 pending → embed + jieba 分词 → 写 vec 行(HNSW 增量)
       → 批末重建被触及的 FTS 索引 → 标 done
```
...
## 4. 一致性关系与校准:file ≥ row ≥ search

写入顺序(files → row → 检索)决定三个面的新旧关系**固定可推理**:

```
file 面  ≥  row 面(DuckDB 事件表)  ≥  检索面(vss + fts 派生表)
```

- **file 面永不缺数据**,至多瞬时超前 row 一步(崩溃后由 repair 收敛)。
- **row 面强一致**(不带 `search()` 的查询永远读到已兑现的行)。
- **检索面最终一致**(consumer 异步兑现;HNSW 增量、FTS 批末重建)。
...
| ② 之后、③ 之前 | 行有、检索派生表没有 | outbox 里仍有 pending,consumer 重启 **replay**(按 pk upsert 幂等) |
| 任意时刻整体损坏 | 派生层不可信 | **rebuild()**:按 `ds` 顺序 replay 全部 `<表>.jsonl` → 重灌事件表 + 重入队检索作业 |

- **`repair`(open 时轻量自检)**:`file ≥ row` 不变式若被打破,从 files replay 补齐。
- **`rebuild()`(显式重建)**:整体从 files 重灌 DuckDB(清空事件表 + 检索派生表,replay 重填;见 [api/admin.md](../api/admin.md))。
...
- **派生 DuckDB 事件表**:**INSERT 一条 del 事件**(pk + `deleted_ds`/`deleted_at` + `_seq`,business 列 NULL)——纯 append,不改任何已写的行。
- **检索派生表**:consumer 收到删除作业 → 从 `_sb_search_<表>__<列>` **DELETE 掉该 pk 的向量行**(检索派生表是可重建投影,非 canonical 历史,可增删)+ 重建该列 FTS → 打了墓碑的行不再被 `search()` 带出。

要点:

- **时光机天然兜住删除**:墓碑事件落删除日;as-of 早于删除日 → 重放到那时最新事件还是 put → 那行仍可见。事件重放对多版本也完备,见 [time_machine.md](time_machine.md)。
- **rebuild 保真**:按 `ds` 顺序 replay,put→INSERT put 事件、`_deleted`→INSERT del 事件(`_seq` 按 replay 顺序,保住时序)。
...
file-canonical 模式不变、且更彻底:文件仍是权威,DuckDB 仍是派生索引。变的是**谁维护文件**——从「各 store 手写文件 ops」收进 seekbase:**每张表自动镜像**成按天分区的 `<表>.jsonl`(不需要 schema 声明落哪、怎么落),写路径自动「文件 → 事件行 → 检索派生表(vss+fts)」,谁也不会忘了哪一边。检索这一层从进程外的 LanceDB 收进同一个 `duck.db`,少一个引擎、少一套 fd 运维(见 [search.md](search.md))。
