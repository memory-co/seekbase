...
seekbase/                      # 仓库根
  ...
  seekbase/                    # 包(flat,不用 src/)
    ...
    _engine/
      ...
      vector.py                # VectorEngine:LanceDB,每个 (表,searchable 列) 一个向量表  [M3 已落]
...
- **`search(列, '文本')` 是 SQL 里的函数**(§4.2 的 `query` 内):`SELECT *, _score FROM cards WHERE search(issue, 'pty tmux') AND kind='issue' ORDER BY _score`,自动 embed + 向量检索(每个 `searchable` 列各自一个向量索引,§6.3),调用方永不见向量。一条 query 可有多个 `search()`(搜不同列),各自暴露一个 `_score_<列>`;单个 `search()` 时额外附便捷别名 `_score`。
...
- **进阶 searchable**:跨表 search、`search()` 融合关键词(hybrid,§见下)。(**多列 search 已做**:每个 searchable 列各自向量索引,`search(列, '文本')`。)
