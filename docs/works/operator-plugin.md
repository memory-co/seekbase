...
>
> **但 seekbase 不自己跑管道**:整条管道被**降级**到一个 runtime(DuckDB `WITH` / bash pipeline,[pipeline-runtime-optimize.md](pipeline-runtime-optimize.md))。算子的**必修课是能算**——给出可移植的 `run`(任何 runtime 都能承载它);**选修课是跑得快**——**可选地**覆写 `optimize_duck` / `optimize_bash`,把自己**原生降级**进某个 runtime,免掉 `run` 那层桥/屏障(§3.2)。名字就点明分工:`run` 必须有,`optimize_*` 是**可选优化**——不写也能跑,写了才更快。
>
...
class Grep(Operator):
    ...
    def optimize_duck(self, prev, args):    # 我在 duck 宿主里长这样(§3.2)
        return f"SELECT * FROM {prev} WHERE {self._sql_pred(args)}"

    def optimize_bash(self, args):          # 我在 bash 宿主里长这样
...
class Operator(ABC):
    ...
    # ── 必修:可移植的兜底实现,任何 runtime 都能承载(§3.2)──────
    #    (ExternalCommand 用 argv 代替它,见 §2 表)
    def run(self, in_data, args, ctx):                raise NotSupported

    # ── 选修:每-runtime 的原生加速,覆写了就免掉 run 的桥(§3.2)──
    def optimize_duck(self, prev, args) -> str:      raise NotSupported   # 可选
    def optimize_bash(self, args) -> list[str]:      raise NotSupported   # 可选
...
| `ExternalCommand(Operator)` | `caps = {Cap.EXEC}`,并**替你实现 `optimize_bash`**(按 `argv` 模板渲染) | 只填 `argv` + 格式 |
...
- **覆写点之间无法共享逻辑**。`optimize_duck`、`optimize_bash`、`run` 三条路要产生**同一个语义**(§3.2),它们必然共用参数校验、谓词构造、列名推导。三个独立函数只能靠模块级 helper 或复制粘贴;有 `self` 就是一个私有方法。
...
`run`(必修基线)+ 两个 `optimize_*`(选修加速),**三者语义必须一致**;加上一对生命周期钩子。逐条看:
...
- **服务型算子(如 `search`)**:背后是一个**常驻服务**——向量引擎连接、**载进 RAM 的 HNSW 索引**、embedder 连接池。这些**开一次、复用多次**,**绝不能每次调用都重开**:
  ...
  | `run` / `optimize_*` | 每次调用 | **复用 `self` 上的 handle**,绝不重开 |
...
### 3.2 可选优化:`optimize_duck` / `optimize_bash`(把自己原生降级进 runtime)

管道不由 seekbase 执行,而是被编译成 **DuckDB `WITH` 链**或 **bash pipeline**([pipeline-runtime-optimize.md](pipeline-runtime-optimize.md))。`run` 是**必修的基线**(任何 runtime 都能靠桥承载它);`optimize_*` 是**可选的加速**——它**在编译期产代码**(返回 SQL 文本 / argv,不碰数据)让这段**原生融进** runtime、免掉桥,而 `run` 是**在运行期真干活**(拿 `_in` 算出结果)。两者语义必须一致(下文):
...
class Grep(Operator):
    ...
    def optimize_duck(self, prev, args):        # 编译期 → 一段 SQL 文本(duck 宿主:翻成 WHERE)
        col = args.field or "*"
        return f"SELECT * FROM {prev} WHERE regexp_matches({col}, {self._regex(args)!r})"

    def optimize_bash(self, args):              # 编译期 → 一段 argv(bash 宿主:就是 grep 本身)
...
> **`optimize_duck` / `optimize_bash` 只是当下的两个 —— 方法名里嵌的是 runtime 名,而 runtime 是开放集。** 管道降级到哪个 runtime(DuckDB / bash / 未来的 WASM、远程…)是[pipeline-runtime-optimize.md](pipeline-runtime-optimize.md)的事;对算子作者,加一个 runtime `R` 就是多一个可覆写的 `optimize_R`,基类不动、老算子不改(没覆写就靠 `run` 在 `R` 里兜底)。别把 runtime 和 `search` 内部的**引擎后端**(LanceDB / duck-vss)混为一谈——那是两层(pipeline-runtime-optimize §1.1)。

**覆写哪几个决定你贵不贵**——由框架**检测覆写**得出(不用声明,和 `kind` 同一原则,§8):

| 你覆写了 | 后果 |
|---|---|
| 只有 `run` | 落哪都靠内联桥承载(duck=vtab、bash=stdin);能跑,**最贵**(优化屏障 + 每批 marshal) |
| 一个 `optimize_*` | 在那个宿主里**免费**;在另一个宿主里是**切换点**(整条管道被劈开) |
| 两个 `optimize_*` | **永不成为切换点**——跟着上下文走,零成本 |

> **写第二个 optimize_* 的理由不是「能在两边跑」,是「不成为切换点」。** 一堆 duck 段中间夹一个只会 bash 的 `grep`,代价是整条管道劈成三截 + 一座 vtab 桥;覆写一个 `optimize_duck`(把 grep 能力整个翻译成 `WHERE`),代价直接归零。收益是复利的,见 [pipeline-runtime-optimize.md §5](pipeline-runtime-optimize.md)。
...
- **`ctx` 只在 `start` 和 `run` 里出现**。`optimize_duck`/`optimize_bash` **拿不到 `ctx`**——它们只生成代码,不碰外界;真正的资源访问发生在宿主里,由沙箱和策略层管(operator-registry §6)。
...
- **同理,「覆写了什么」也是推导的,不是声明的**:有没有常驻状态看有没有覆写 `start`;能进哪个宿主看覆写了哪个 `optimize_*`。**一律不设声明字段**——声明和实现能打架的地方,就是 bug 的产地。
...
> | **宿主降级** | 覆写了哪几个 `optimize_*` | 我能编译进哪个宿主? | 宿主指派 + 会不会成为切换点(§3.2) |
>
> ```
> Grep         = TABLE→TABLE   + 无状态      + optimize: duck & bash   永不成为切换点
> LanceSearch  = NONE →TABLE   + 服务型      + optimize: duck & bash   duck 走官方集成,bash 走 SDK
> Jq           = JSONL→JSONL   + 无状态      + optimize: bash         在 duck 宿主里是切换点(架 vtab)
> 你的第一个算子 = TABLE→TABLE  + 无状态      + optimize: 无           只覆写 run:能跑,最贵
> SQL 段        = TABLE→TABLE   + (非 plugin) + 天生 duck              整条链的锚点
> ```
>
> **最低门槛**:继承 `Operator`,实现 `out_schema` + `run`,收工。`start`/`stop` 和 `optimize_*` 都是**为了跑得便宜才覆写的第二档**。
...
class Grep(Operator):
    ...
    def optimize_duck(self, prev, args):
...
class LanceSearch(Search):                                # 后端 A:LanceDB
    ...
    def optimize_bash(self, args):                        # bash 宿主:小命令直调 LanceDB SDK
        return ["seekbase-search", args.table, args.text, "--k", str(args.k)]

    def optimize_duck(self, prev, args):                  # duck 宿主:DuckDB×LanceDB 官方集成
...
class VssSearch(Search):                                  # 后端 B:DuckDB-vss + fts,同一父类
    ...
    def optimize_duck(self, prev, args):                  # 本来就在 duck 里,最自然
...
用:`search cards '事故' | jq 'select(.severity>=3)' | SELECT kind, count(*) FROM _in GROUP BY kind`。默认 `read-only` 策略下 **`jq` 因 `EXEC` 被拒**(`ExternalCommand` 预设了 `caps={Cap.EXEC}`),要显式升级到 `sandboxed`/`trusted` 才能跑(operator-registry §6)。**只有 `optimize_bash`** ⇒ 它在 duck 宿主里是个切换点,编译器会为它架一座 vtab 桥。
...
authorize                算子 caps vs 策略 → allow / ask / deny(deny→编译期拒,管道不启动)
   │
compile                  宿主指派 → 调 optimize_duck / optimize_bash 生成代码;都没有就包 vtab 调 run
   │
...
- **多路 differential test(准入条件,非可选)**:同一份输入,分别**强制**走 `optimize_duck` / `optimize_bash` / `run`,断言逐行一致。三条路结果不同 = 同一条 query 换宿主换答案,这是本设计最危险的 bug(§3.2)。**父类可以直接提供这个测试基类**,子类只填输入——这是继承的又一处兑现。
...
- **降级测 `EXPLAIN`**:断言宿主指派符合预期(比如「加了 `optimize_duck` 之后切换点从 2 变 0」),否则性能回退无人察觉。
...
- **多路实现 = 多份维护 + 等价性风险**(§3.2)。只覆写一个 optimize_* 是完全正当的选择,它只意味着这个算子**是个切换点**——请在文档里写明,别让用户猜为什么某条管道突然变慢。
- **只覆写 `run` 不是错,是贵**:保底 vtab 一定跑得通,但它是**优化屏障**(DuckDB 看不穿桥、谓词推不进去、基数估计失真)+ 每批 marshal。热路径上的算子值得补一个 optimize_*。
...
- [pipeline-runtime-optimize.md](pipeline-runtime-optimize.md):**本文 `optimize_*` 的去处**——宿主指派、融合切段、vtab 桥、代价阶梯。作者视角看「覆写几个 optimize_*」,系统视角看「编译器怎么用它们省钱」。
