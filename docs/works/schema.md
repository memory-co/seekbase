...
- 修饰 `primary`:**每表恰一个主键**。主键是三引擎对齐的锚——DuckDB 行、LanceDB 文档、文件路径都按它对齐(见 §8)。主键只能是标量 `str` / `int`,不能是 `json` / `decimal` / `timestamptz`。
...
| 主键是 `str` / `int`(非 `json` / `decimal` / `timestamptz`) | `SchemaError` |
