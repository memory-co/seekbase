...
两种镜像模式,**由 schema 声明,不由 seekbase 猜**——seekbase 业务无关,不知道「这是 rounds 所以该 append」。选哪种的判据是**纯结构性**的:路径模板和行是 1:1 还是 1:多(这同时是正确性条件,见下)。

- **json 模式(一行一文件)** —— 路径模板 **1:1 于行**,即模板含**主键**,每行解析出唯一路径。
  - 例:`files: "cards/{card_id}.json"`(`card_id` 是主键)→ 每行独占一个文件,目录按 id 可导航。
  - 内容是 **pretty-printed、键序稳定** 的 JSON——写一次就不动(引擎 insert-only),diff 友好。
  - **正确性**:per-row 文件必须由唯一键定位,否则两行会撞同一个文件——所以模板**必须含主键**。

- **jsonl 模式(追加)** —— 路径模板 **1:多于行**,即模板按**非主键分组列**,多行共享一个路径。
  - 例:`files: {path: "sessions/{session_id}/rounds.jsonl", mode: "jsonl"}`(`session_id` 非主键)→ 同组多行追加进一个文件,不炸成十万个小文件。
  - **正因为键不唯一**,同一路径会来多行,只能 append-only 追加。

> 判据落到机制上就一句:**模板里放的是主键还是分组键**。seekbase 甚至能据此自动判(含主键→json,否则→jsonl),但为消歧仍让它显式声明——或直接由路径扩展名给(`.json` / `.jsonl`)。**全程不碰业务含义。**
