...
- **一个执行器抽象撑起两形态**(`_engine/executor.py`):`LocalExecutor` 把 `Request` 派给 `service/`(query/write/admin)并把写结果包成 ticket;`HttpExecutor` 把同一个 `Request` 序列化成对应的 HTTP 端点调用发给 server。`port` 只构造 `Request`,不认识自己跑在哪种形态。分层:`port → executor → service(编排)→ _engine(机制:duck/search/files)`。
