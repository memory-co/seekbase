...
|---|---|---|---|---|---|
| **① 同宿主原生降级** | 算子实现了当前宿主的 `compile_*` | 无 | ✅ 全可见 | 无 | **0** |
...
```sql
WITH s0 AS (SELECT * FROM lance_search('cards','…')),      -- search.compile_duck
     s1 AS (SELECT * FROM s0 WHERE regexp_matches(…)),      -- grep.compile_duck ← 关键:没用 vtab
...
**(c) `grep` 有 `compile_duck` 省掉的那次 vtab**

```
search … | grep ERROR | SELECT … | SELECT …
```
- `grep` **只有** `compile_bash` → 指派成 `duck,bash,duck,duck`,2 次切换,中间要架一座 vtab 桥。
- `grep` **有** `compile_duck`(把 grep 能力整个翻译成 `WHERE regexp_matches(...)`)→ 全段 `duck`,**0 次切换、0 座桥**,而且 DuckDB 优化器能把这个 `WHERE` 和上下游一起优化(甚至下推进 `search` 的表函数)。

> 这就是**为什么值得给一个算子写两版 compile_**:不是为了「能在两边跑」,是为了**让它不成为切换点**。一堆 duck 段中间夹一个只会 bash 的 `grep`,代价是整条管道被劈开;给它一版 `compile_duck`,代价归零。

## 6. 算子要交的作业:两个 compile_*
...
class Grep(Operator):
    ...
    def compile_duck(self, prev, args):        # duck 宿主:整个翻译成 WHERE
        return f"SELECT * FROM {prev} WHERE regexp_matches({args.field}, {self._regex(args)!r})"

    def compile_bash(self, args):              # bash 宿主:就是 grep 本身
...
|---|---|
| `run` | 永远走 ③ vtab;能跑,最贵 |
| 一个 `compile_*` | 在那个宿主里免费,在另一个宿主里是**切换点** |
| 两个 `compile_*` | **永不成为切换点**——跟着上下文走,零成本 |

### 6.1 `search` 的两版(两条都是真实的实现路径)

| | `compile_bash` | `compile_duck` |
|---|---|---|
...
- **Python `run` 算子同理**:没有任何 `compile_*` 的算子,由框架包成 duck 侧的 vtab(批回调就是它的 `run`),或包成 bash 侧的一个子命令。

## 8. 语义等价是作者的责任(必须有 differential test)

一个算子的两个 compile_* **必须产生相同结果**,否则同一条 query 因为编译器选了不同宿主而**结果不同**——这是这套设计最危险的一类 bug,而且极难查。
...
- **强制 differential test**:同一份输入,分别强制走 duck 版和 bash 版,断言逐行一致。这是双 compile_* 算子的**准入条件**,不是可选测试。
