...
seekbase/                      # 仓库根
  ...
  seekbase/                    # 包(flat,不用 src/)
    ...
    _engine/
      ...
      duck.py                  # DuckdbEngine:单写者连接、每业务表一张物理表(写一次 PK)、可见性视图、search 重写
      search.py                # SearchEngine:业务表上的 vss(HNSW)+ fts(BM25),inline embed、RRF 融合
      ...
      # 写同步:insert 内联 embed+jieba → 随行 INSERT(含 _vec/_tok)→ 同步重建 FTS(无 outbox/consumer)
      # search() 重写在 duck.py(extract_searches)+ executor(RRF 检索→join)
      # 时光机 = duck.py 单表 ds 谓词(ds/deleted_ds);ds_start/ds_end 是 query 参数
    embedders/
