...
- 分片深度记在 `_meta.json`,`rebuild()` 一致复算。`vacuum(before=D)` = **整块 `rm -rf` 掉 `ds < D` 的分区目录**,比逐行清墓碑干脆。
...
`delete()` 打墓碑,**不删也不改原文件**——墓碑是**当天分区(`ds=删除日`)里追加的一条删除记录**(如 `{"_deleted": "<pk>", "table": "cards", "deleted_at": "…"}`),不回头改原行所在的旧分区文件。

- **纯 append,原文件永不变**:原行还躺在它的创建日分区里、原样不动;liveness 由「`ds <= D` 的分区里有没有它的墓碑记录」判定。这比在旧文件里就地改 `deleted_at` 更契合分区模型,也让「已存在文件永不变」这条不变式**没有例外**。
- **时光机天然兜住删除**:墓碑记录带 `ds=删除日`,`ds_end` 裁掉更晚的分区 → 晚于窗口的删除自动不可见,不需额外谓词。
- **grep 也看得见**:`grep -r _deleted files/ds=20260706/` 就能看当天删了什么、何时删。

真正物理删只发生在显式 `vacuum(before=D)`——整块删掉 `ds < D` 的分区目录(含那些天的墓碑记录),**明说这是在丢历史**。
