...
- **`search(列, '文本')` 是 SQL 里的函数**(§4.2 的 `query` 内):`SELECT *, _score FROM cards WHERE search(issue, 'pty tmux') AND kind='issue' ORDER BY _score`,自动 embed + 向量检索(每个 `searchable` 列各自一个向量索引,§6.3),调用方永不见向量。
