...
| [pipeline-runtime-optimize.md](pipeline-runtime-optimize.md) | **设计稿(后端)**:**管道不自己跑**——整条被降级成 DuckDB 的 `WITH` 链或 bash 的 pipeline,于是跨段优化 / 背压都是白送的。三种降级手段的代价阶梯(**同宿主原生翻译 0 成本 > 切段一次物化 > vtab 桥每批 marshal**)、宿主指派 = 一条最短路、连续同宿主段融合、`grep` 多写一版 `native_duckdb`(翻成 `WHERE`)如何把切换点消成 0、`search` 的两版 native(bash 调 LanceDB SDK / duck 走官方集成);外加双 native 的语义等价风险(differential test 是准入条件)|
| [operator-plugin.md](operator-plugin.md) | **设计稿(作者视角)**:一个算子 = 一个 **`Operator` 子类**——声明放类属性、行为放方法覆写,层次只有一层(`Source` / `ExternalCommand` 只预设默认值)。**为什么是继承不是记录**:服务型要 `self` 存 handle、三条降级路要共享私有方法、一父两子复用(`Search` → `LanceSearch`/`VssSearch`)、`parse_args` 按参数变 caps。**三个正交轴,全从「覆写了什么」推导**:① 格式契约 `accepts`/`emits`(不用 `kind`)+ coercion;② 覆写 `start`/`stop` = 服务型;③ 覆写哪几个 `native_*` = 能进哪个宿主、会不会成为**切换点**(都不覆写走保底 vtab——能跑最贵)。外加有界性塌缩成宿主属性(**`tail -f` 进 duck = 编译期报错**;watermark/checkpoint/keyed state 明确不借)、`ctx` capability 唯一入口、三个例子(`Grep`/`Search`/`Jq`)+ 测试 + 诚实代价 |
