...
三个键:`columns`(必)、`searchable`(可选)、`files`(可选)。
...
- 修饰 `primary`:**每表恰一个主键**。主键是三引擎对齐的锚——DuckDB 行、LanceDB 文档、文件路径都按它对齐(见 §8)。
- v1 只有这四种标量类型 + 一个主键修饰;够记忆系统用,不引入 FK / 复合主键 / 嵌套(关系走扁平列 join,容忍悬挂,不加外键)。
...
- 列出可被 `search()` 语义检索的列。声明了 → `insert` 时该列文本**自动 embed** 进向量侧、`search()` **自动查**——写入方 / 查询方都不用碰第二条(向量)栈。
