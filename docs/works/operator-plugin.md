...
> **无界流 / `tail -f` / 流式语义留给专文(待写)细谈**,这里只钉最小的一条编译期规则,不展开。
...
- **无界测编译期**:bash runtime 的无界 source 接一条 SQL 段,断言**编译期**报错(而不是跑起来挂住,§3.3;流式细节留给专文)。
...
- **今天 seekbase 没有一个无界 source**。所以 §3.3 那条检查现在的收益只有:`sh 'tail -f'` 从「挂死」变成编译期报错(流式细节留给专文)。
