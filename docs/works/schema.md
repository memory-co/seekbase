...
```
                         SCHEMA(唯一声明源)
        ┌───────────────┬───────────────┬───────────────┐
   DuckDB 事件表     DuckDB vss+fts    files 镜像      元数据列
   (结构化 DDL)      检索派生表        (canonical)     ds/created_at/
   建表/列/主键      (若有 searchable) (每表自动)      deleted_ds/deleted_at
                     每列一套向量+全文                  → 时光机
```

上层(repository / service)从此**不手写 SQL、不手写文件 ops、不手写检索接线**——加一列 = 改一处 schema,DDL / 迁移 / 文件+检索派生全由 seekbase 兑现。检索(向量 `vss` + 全文 `fts`)和结构化**同在一个 `duck.db`**(单引擎,无 LanceDB)。文件镜像是**每张表自动**落成按天分区的 `<表>.jsonl`,不用在 schema 里声明(见 [store.md](store.md))。
...
- 是**各层对齐的锚**:DuckDB 事件行 / 检索派生表的 `pk` / 文件里的行都按它对齐(见 §7)。文件里主键**不做文件名 / 不进路径**——只在派生层做行标识、多版本现算最新、删除定位(见 [store.md §2.1](store.md))。
...
- 列出可被 `search()` 检索的列;**必须是 `str` 列**(向量只 embed 文本、BM25 只索引文本,`json` / 数值 / 时间列不可搜)。声明了 → `insert` 时该列文本**自动 embed + jieba 分词**进检索派生表(vss+fts)、`search()` **自动 hybrid 查**——写入方 / 查询方都不用碰检索栈(见 [search.md](search.md))。
- **有 `searchable` ⇒ 必须注入 embedder**(否则 `open` 时 `EmbedderInvalid`,早失败)。
- **没有 `searchable` 的表 = 纯结构化表**:零检索开销,连 `_sb_search_<表>__<列>` 派生表和 vss/fts 索引都不建。
- 这就是「`search()` 一等算子」的接线开关:声明与否决定一张表进不进检索侧(vss+fts)。

## 7. 一张 schema 表 → 落地(单引擎 DuckDB + 文件)

以 `cards`(`issue` 可搜)为例,`open` 时从这一条声明落成:

| 目标 | 落地 |
|---|---|
| **DuckDB 事件表** | `CREATE TABLE _sb_cards (card_id VARCHAR, issue VARCHAR, kind VARCHAR, ds, created_at, deleted_ds, deleted_at, _seq BIGINT)` —— **append-only 事件表**(无主键约束:重复主键 = 多版本;查询走重放视图) |
| **DuckDB vss+fts 派生表** | 每个 `searchable` 列一个 `_sb_search_cards__issue(pk, txt, tok, vec FLOAT[dim])`;`vss`/HNSW 索引 `vec`、`fts`/BM25 索引 `tok`;`search(issue, …)` 只搜这一列、hybrid 融合(见 [search.md](search.md)) |
| **files** | `ds=<写入日>/cards.jsonl`,每次写 append 一行(每表自动,见 [store.md](store.md)) |

**主键 `card_id` 是各层对齐的锚**:一次 `insert`,DuckDB 事件行 / 检索派生表的 `pk` / 文件里的行都以它对齐,id 对齐由引擎在端口后面兜。
...
- [search.md](search.md):`searchable` 如何接线 vss+fts 派生表、hybrid 检索。
