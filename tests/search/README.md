...
`_score_<列>` 列(单个 search 也附便捷别名 `_score`),和结构化过滤 / 时间窗写在同一条 `query` SQL 里,全在 **DuckDB 单引擎**内(无 LanceDB)。检索派生表由后台
consumer 从 outbox 异步兑现(HNSW 增量、FTS 批末重建),`wait(ticket)` 排干。
...
6. **rebuild 后仍可搜**:`rebuild()` 清空并重灌检索派生表,consumer 重填后 `search()` 照常(`test_rebuild_repopulates_search`)。
