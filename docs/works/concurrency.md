...
## 2. 动因:一条线程把「读」也串行了(readpool 之前的问题)

> 这一节讲的是 **readpool 落地之前**的状态——为什么需要读写分离。解决方案(读走 cursor 池、不再排在写后)见 §3,已落。

起初 Bridge 串行化**一切** DuckDB 访问——`query` 也走 `bridge.run`。于是**读排在写后面**:一次**检索管道**会卡在前面的写、尤其是 **FTS 重建**(duck-vss 后端每次 insert 同步重建,`create_fts_index` 是 O(表大小),大表几百毫秒~秒级;lance 后端无此步)之后。对读多写少的 memory 场景,这曾是主要的并发痛点——正是 §3 要解决的。
...
- **执行 + 记录在一处**:队列的「多→一」是串行/单写者,循环末尾追加**出生即 done 的 task**(原 ticket)是完成标记——一次写的执行在 `WriteService`,记录追加进共享的 TaskService 日志(见 [task.md](task.md))。
...
- `WriteService`(`service/write_service.py`)一个组件吃下写的全部:校验/embed/顺序编排 + worker 串行执行 + 末尾追加出生即 done 的 task;`Bridge` 留 `runtime/`(通用原语,读写两侧都用)。
- `rebuild`(`admin_service`)是最重的写,现在是 **TaskService 的后台 task**(pending→done,见 [task.md §3](task.md)),其中的阻塞库调用仍经 Bridge 串行。读侧无 task、无 worker,直接走读 cursor;显式 `as_task` 的查询例外——它也是后台 task,但执行仍走读 cursor。
...
- [task.md](task.md):写的完成标记 = 出生即 done 的 task,由 WriteService 的 worker 末尾追加;rebuild/慢查询是真 pending→done 的后台 task。
