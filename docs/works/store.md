...
- **顶层日期分区 `ds=YYYYMMDD`**(离线大数据惯例):每行按写入日落进对应分区。**时光机就是分区裁剪**——as-of D 读 `ds <= D` 的分区(见 [DESIGN §6.5](../../DESIGN.md));`ls files/ds=20260705/` 直接看当天全部数据,审计 / 备份 / 回看某天都落成「看一个目录」。
...
- **时光机天然兜住删除**:墓碑记录带 `ds=删除日`,as-of D 裁掉 `ds > D` 的分区 → 晚于 D 的删除自动不可见,不需额外谓词。
