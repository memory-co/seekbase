...
| | json 模式 | jsonl 模式 |
|---|---|---|
| **路径怎么来** | `ds=<写入日>/` + 模板 `{col}` 用该行列值填充 + PK 哈希分片(§2.3)| 同左,但填充用**分组列**,分片按分组键 |
| **一个文件装什么** | **这一行**的完整 JSON 对象(全部声明列 + 元数据 `ds`/`created_at`;删除信息在删除日分区的墓碑里),pretty-print、键序稳定 | 同组**多行**,**一行一条** compact JSON,按 insert 顺序 append |
| **怎么定位一行** | 由 PK 直接算出路径 → 打开那一个文件(O(1)) | 由分组列算出文件 → 顺扫其行、按 PK 命中(同 PK 多条时**最后一条为准**)|
| **删除(墓碑)** | **当天分区**追加一条删除记录(`{"_deleted": "<pk>", …}`),原文件不动(§5)| 同:当天分区追加一条墓碑记录 |
...
**解法:顶层按日期分区 `ds=YYYYMMDD`,分区内再按 PK 哈希分两级分片**(git object store 的路数),seekbase 自动加、业务无感:

```
<data_dir>/
  files/
    ds=20260705/                             # 顶层 = 日期分区(写入日);当天数据全在这
      cards/                                 # json:分区内按 PK 哈希分片,单目录条目恒有界
        3f/                                  # = hash(card_id)[0:2]
          a1/                                # = hash(card_id)[2:4]
      ...
      sessions/                              # jsonl:分组键多时同样分片
        9c/ e2/
          sess_1/
            rounds.jsonl                     # 同 session 当天的多行 append 进来
    ds=20260706/                             # 次日一个新分区目录
      ...
  duck.db                                    # DuckDB:业务行(带 ds 列)+ _outbox 队列表
  lance/                                     # LanceDB:每个有 searchable 列的表一个 collection
  _meta.json                                 # 实例元:schema 指纹、版本、embedder dim
```

- **顶层日期分区 `ds=YYYYMMDD`**(离线大数据惯例):每行按写入日落进对应分区。**时光机就是分区裁剪**——`query` 的 `ds_start`/`ds_end` 圈定要读的分区区间(只给 `ds_end` = 回到那天及之前;见 [DESIGN §6.5](../../DESIGN.md));`ls files/ds=20260705/` 直接看当天全部数据,审计 / 备份 / 回看某天都落成「看一个目录」。
- **分区内再分片**:分片键 = `blake2b(主键值)` 前 4 个 hex,两级各 256 路 → **65536 个叶目录**;叶目录约 `当天行数/65536` 个文件。日期分区先按天切、分片再把单日切匀——**任何单目录条目数都有界**,不碰 FS 上限、不拖垮 readdir。
- **仍可 grep / 定位**:给定 id + ds 就能算出分片路径(O(1) 打开);`grep -r files/ds=20260705/` 整树递归照跑。
- **jsonl 模式**多行归一文件、天然少扇出;分组键基数大时,分区内对分组键套同一分片。
- 分片深度记在 `_meta.json`,`rebuild()` 一致复算。`vacuum(before=D)` 按行清 `deleted_ds < D` 的死行(见 §5 / [time_machine.md §8](time_machine.md)),**不是**整块删分区。
...
```
① files 最先 —— 原子落盘(目标路径 = `ds=写入日/` + §2.3 分片,按需先 mkdir -p)
...
- **insert-only**:已存在的文件**永不再变**(删除也是新分区里追加一条墓碑记录,不回改旧文件,§5),读者拿到的结果稳定、可缓存。
...
对账也靠 files:DuckDB 原生 `read_json` / glob 能把 files 当外部表,直接与 row 面 diff(`SELECT * FROM read_json('files/ds=*/cards/**/*.json', hive_partitioning=true)`——`ds` 自动成为一列)。

## 5. delete / 墓碑:当天分区里的一条记录

`delete()` 打墓碑,**不删也不改原文件**——墓碑是**删除日分区(`ds=删除日`)里追加的一条删除记录**(`{"_deleted": "<pk>", "src_ds": "<创建日>", "deleted_at": "…"}`,`src_ds` 指回原行所在的创建分区),不回头改原行文件。

- **纯 append,原文件永不变**:原行还躺在它的创建日分区里、原样不动;删除是删除日分区里的一条新事件。「已存在文件永不变」这条不变式**没有例外**。
- **派生行记两对日期**:DuckDB 派生行带 `ds`/`created_at`(创建)+ `deleted_ds`/`deleted_at`(删除,消费墓碑事件时置上)。时光机判定 `ds <= D AND (deleted_ds IS NULL OR deleted_ds > D)` —— 完整设计与完备性证明见 [time_machine.md](time_machine.md)。
- **grep 也看得见**:`grep -r _deleted files/ds=20260706/` 就能看当天删了什么。

真正物理删只发生在显式 `vacuum(before=D)`——**按行**清掉 `deleted_ds < D` 的死行(创建文件 + 墓碑 + 派生行 + 向量);**不是**整块删分区(那会误删「早创建但仍活」的行)。活行、以及删于 `≥ D` 的行都保留。代价:vacuum 后只能倒带到 `≥ D`。见 [time_machine.md §8](time_machine.md)。
