...
**包的对外承诺(端口契约)**:调用方只见 SQL `query`(含 `search()`)+ 异步写(`insert`/`delete` + ticket)+ 少数管理动作(`rebuild/vacuum/close`)。**永远看不见**:向量、embedding 计算、outbox、consumer、DuckDB 连接句柄、Lance 目录结构、文件落盘顺序。这条「不漏进程内假设」的纪律也是云端版能直接换 HTTP 的前提(§9)。
...
- **自动滤墓碑**:引擎给每张表挂一个可见性视图(`deleted_ds IS NULL …`);`ds` 窗口按分区裁剪。
...
- **写是异步的**:提交返 `ticket`,`write_status`/`wait` 查状态(files→行→向量;M1 同步兑现,即返 `done`)。
- **焊死不变性(引擎强制)**:端口无 `update`/`upsert`;`delete` 唯一语义写 `deleted_ds` 墓碑;重复主键 = 追加新版本(latest-wins)。
- **`search()` 是 SQL 里的函数**(§4.2 的 `query` 内):`SELECT *, _score FROM cards WHERE search('pty tmux') AND kind='issue' ORDER BY _score`,自动 embed + 向量检索(§6.3),调用方永不见向量。`[M3]`

### 4.4 管理动作

```python
await db.rebuild()               # 按 ds 顺序 replay 全部 <表>.jsonl → 重灌 DuckDB + LanceDB(M2)
await db.vacuum(before="20260601")   # 显式丢历史:按行清 deleted_ds < before 的死行(M4)
```

- `rebuild`/`vacuum` 也返回 `ticket`(异步)。都是「派生层可重建、canonical 在文件」的兑现。
...
- **server 极简、无框架**(`server.py`):手写 ASGI app,路由 `POST /v1/{query,insert,delete,rebuild,vacuum}` + `GET /v1/{writes/{ticket},health}`。**server 标配 = `seekbase_server(db)`,零第三方依赖**;跑它的 **ASGI runner(uvicorn/hypercorn/…)由外部注入**——挂进自己的服务、或用便捷函数 `serve(db, runner=…)`(`runner` 默认取 uvicorn,前提是宿主装了)。client 端只需 httpx(核心已带)。测试用 httpx 的 in-process `ASGITransport` 打全链路,不需任何 runner。
...
- **时光机细节**:用谁的钟、粒度(天够不够)、`ds_end` 但向量还在队里的边界、vacuum 默认策略。
