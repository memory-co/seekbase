...
> **物化是缺省,不是唯一。** 上面写的都是「上一段全算完 → 物化成 `_in` → 下一段开始」,这对有界的小结果集(`search` 的 top-k)完全够。但它假设了**上游一定会结束、且全量装得下**——`scan` 大表要吃全表内存、下游 `LIMIT 20` 不能早停、`sh 'tail -f …'` 更是**静静挂死**。参考 Flink 的算子设计,`_in` 也可以是挂在上游 chunk 流上的 **Arrow reader**(分块推 + 早停 + 取消反向传播),并给流声明**有界性**把「挂死」变成编译期错误。这是算子契约的一部分,见 [tool-plugin.md §3.2/§3.3](tool-plugin.md)。
