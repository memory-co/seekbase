...
管道文法极简:`pipeline := stage ('|' stage)*`;一个 `stage` **要么是一条 DuckDB SQL**(transform),**要么是 `verb args`**(source 如 `search`/`scan`、tool 如 `sh`/`http`)。判据就是 §2.1:在 DuckDB 里能表达的写成 SQL 段,跨引擎/跨进程的才用 verb 段。编译分工:
