...
| [tool-plugin.md](tool-plugin.md) | **设计稿(作者视角)**:要写一个工具,得实现什么 plugin?核心契约 `run(in_data, args, ctx) → out_data`;**格式契约 `accepts`/`emits`(不用 `kind`——source/sink 的位置从格式推导)** + 格式 coercion;两个正交轴:格式 ⊥ **无状态 vs 服务型**(`grep` 无常驻只 `run`,`search` 带常驻引擎/RAM 索引 `start`/`run`/`stop`);`ctx` 是 capability-based 唯一外界入口、诚实声明 caps + 输出 schema;三个例子从简到繁(`grep`/`search`/`jq`)+ 测试 + 诚实代价 |
